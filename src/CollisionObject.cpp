...
CollisionObject::CollisionObject(char* fileName, int* rotationOrder, float rightBound, float frontBound)
...
CollisionObject::CollisionObject(char* fileName, float rightBound, float frontBound)
	: DrawObject(fileName)
{
	this->rightBound = rightBound;
	this->frontBound = frontBound;
...
bool CollisionObject::CollidesWith(CollisionObject* other)
{
	...
	float myWidth = this->rightBound * 2;
	float myHeight = this->frontBound * 2;
